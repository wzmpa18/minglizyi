...
> 最后更新: 2026-08-24（生产版本 v25.0.47_23 + APK v25.0.52：FIX-V23-MEMBER-BUY-GUIDE 会员购买引导即时可见——①「立即开通」按钮由文档流底部改为 **fixed 悬浮底栏**（position:fixed+bottom:calc(底部导航高度+safe-area-inset-bottom)+zIndex 1001+上投影），用户在会员页**任何滚动位置**（含刚点击会员卡片后）无需滚动即见购买引导按钮，132px 文档流占位防遮挡页面尾部内容；②中医区「升级会员解锁全部中医功能」入口滚动逻辑修复（window.scrollTo 顶部→plan-section 锚点 scrollIntoView 精确滚到套餐区）；③公网 E2E 实测 17 PASS/0 FAIL（真实账号 13612674128：登录协议勾选→会员页按钮视口内 y=782 可见→点击卡片按钮文案即时联动「立即开通 · ¥37」→滚动 1800px 按钮 fixed 位置恒定→点击开通支付二维码弹窗+价格正确+取消支付可关闭→四档位价格 SSOT 动态断言 37/99/374/3600 全联动→未登录点击弹出登录引导）；前一版本 v25.0.47_22：MARKETING-POSTER-V2-AI 邀请裂变海报营销化升级+AI 智能文案生成（三套社交裂变模板+渲染引擎视觉规范+海报 3:4+AI 换文案+分享文案库三场景）；指令 FIX-V23-MEMBER-BUY-GUIDE）
...
| 当前生产版本 | **v25.0.47_23**（buildId v25.0.47_23_D20260824，2026-08-24 发布：FIX-V23-MEMBER-BUY-GUIDE 会员购买引导即时可见——①**「立即开通」fixed 悬浮底栏**：按钮由文档流底部改为 position:fixed+bottom:calc(var(--bottom-nav-height,56px)+env(safe-area-inset-bottom))+zIndex:1001+上投影 boxShadow，会员页任何滚动位置（含刚点击会员卡片后）无需滚动即见购买引导，132px 文档流占位（aria-hidden）抵消 fixed 栏高度防遮挡页面尾部内容；②**中医区升级入口滚动修复**：会员状态卡「升级会员解锁全部中医功能 →」点击由 window.scrollTo({top:0}) 改为 document.getElementById("plan-section")?.scrollIntoView 精确滚到套餐区（套餐标题 div 新增 id="plan-section"）；③**公网 E2E 实测 17 PASS/0 FAIL**（scripts/test_membership_v23_live.js，真实账号 13612674128：登录协议勾选→按钮视口内 y=782 可见→点击卡片文案即时联动→滚动 1800px fixed 位置恒定→支付二维码弹窗+价格 ¥37+取消支付可关闭→四档位价格 SSOT 动态断言 37/99/374/3600 全联动→未登录点击弹出登录引导）；APP 经 Capacitor server.url 直载线上页面，v23 网页部署即覆盖 APP 端无需重建 APK |
| Git HEAD | v25.0.47_23 代码提交 5ad3a37（main，功能提交+总账收口提交；本地=GitHub=服务器源码仓=生产运行目录 四端一致；会员购买引导 fixed 悬浮底栏+中医区滚动修复+发布脚本+E2E 测试脚本三份[取证/本地预演/公网实测]） |
...
| 前端发布 | /root/yandaoguoxue/releases/&lt;tag&gt; + current 软链（SPA 静态导出）；当前 current→v25.0.47_23，回滚目标 v25.0.47_22；目录现状 _5/_6/_8/_9/_17/_21/_22/_23 等多版（磁盘充足暂留，如需清理保留 _22+_23 即可） |
