...
> 最后更新: 2026-08-25（生产版本 v25.0.47_29 + APK v25.0.55(2055)：FIX-V29-DOWNLOAD-RESCUE 升级下载链路根治——用户三次报告「弹窗提示升级点击后对话框多次打开自己消失，正在添加好友转圈+正在下载 Toast 几分钟无进展」截图确诊三叠加缺陷：①壳内 WebView 无 DownloadListener，location.href 赋 apk 地址被静默吞掉（「正在下载」永远不动根因）——修复：/friend、/download 壳感知下载改 intent:// URI（WebView shouldOverrideUrlLoading 交系统 ACTION_VIEW 拉起系统浏览器下载）+「下载没反应？复制下载链接」兜底按钮；②/friend「正在添加好友」假转圈——升级跳转无 ref 参数时 autoAddFriend 不执行但卡片永久显示加载态，改为仅 referrerId 存在才显示加好友卡；③升级弹窗重复弹——handleUpgrade 点击即 sessionStorage 标记+setDismissed，返回 APP visibilitychange 不再重复弹窗；原生层永久修复：MainActivity 新增 DownloadListener（系统 DownloadManager 接管壳内下载，通知栏可见），APK v25.0.55(2055) 内置 v29 资源+dex 符号验证入包；web v29 纯前端发版即时救活全部老壳/内置壳存量用户；单一分发源 latest.apk 三文件同 MD5 bf050537 分发+升级配置 2055+公告栏 v25.0.55 置顶；前一版本 v25.0.47_28：FIX-V28-LEGACY-SHELL-RESCUE 老壳救援三通道探测）
