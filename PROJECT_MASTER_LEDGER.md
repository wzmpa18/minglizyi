...
> 最后更新: 2026-08-23（生产版本 v25.0.47_15：收尾检查发现并修复下载APP链路APK直链404死链——FINAL-SEAL-03品牌统一删除旧APK文件但friend落地页/shareService代码引用未同步，新用户扫码注册后「立即下载APP」404；服务器在线热修（补挂别名文件，无需重打包APK）+代码统一正式包名+完整发布回归，指令 FIX-V15-APK-DEADLINK，代码提交 f65f498→脚本修正 df45284→总账收口，四端一致）
...
| 当前生产版本 | **v25.0.47_15**（buildId v25.0.47_15_D20260823，2026-08-23 发布：FIX-V15-APK-DEADLINK 收尾修复——下载APP链路APK直链404根除：根因系FINAL-SEAL-03品牌统一时删除服务器旧APK文件 guoxue-chuancheng-v1.0-release.apk 但 src/app/friend/page.tsx 与 shareService.ts 代码引用未同步更新，导致新用户扫码注册→「立即下载APP」404；修复三管齐下：①服务器在线热修（/var/www/yandao.vip/app-download/ 补挂别名文件指向v25.0.47正式包，存量分享链接继续有效，**无需重新打包APK**）②代码统一正式包名 yandao-guoxue-v25.0.47-release.apk（与/download页一致）③删除shareService.ts未使用的失效常量；公网验证：正式包直链200+别名兼容直链200+MIME=application/vnd.android.package-archive，注册/登录页下载按钮三环境（桌面Chrome/iOS Safari UA/微信UA）可见性复验通过，支付四环境下单回归19/19 PASS，20页面全200） |
| Git HEAD | 总账收口提交（main，本地=GitHub=服务器源码仓=生产运行目录 四端一致；v25.0.47_15 代码提交 f65f498[APK死链修复+版本号]+df45284[发布脚本校验关键词修正]，其上为本总账收口文档提交） |
| 正式 APK | https://yandaoguoxue.yandao.vip/app-download/yandao-guoxue-v25.0.47-release.apk（MD5 d0b4d90857ffce0edb4c89daf6c75ce4，10.82MB，v2 签名）；服务器另挂别名文件 guoxue-chuancheng-v1.0-release.apk（同MD5，兼容存量分享海报/链接，v25.0.47_15起前端代码已统一指向正式包名） |
...
| 前端发布 | /root/yandaoguoxue/releases/&lt;tag&gt; + current 软链（SPA 静态导出）；当前 current→v25.0.47_15，回滚目标 v25.0.47_14；目录现状 _5/_6/_8/_9/_10/_12/_13/_14/_15 九版（磁盘充足暂留，如需清理保留 _14+_15 即可） |
