...
> 最后更新: 2026-09-01（十二.31 批次：**v25.0.74 用户反馈双修复上线——排盘记录云端保存链路修复（clientStore 死路径→/api/auth/records/save 正式链路+离线队列登录补传+13 工具页去 selectedClient 守卫+records 页 22 类型映射）+ 安卓返回手势修复（MainActivity OnBackPressedCallback：右滑/返回键→WebView goBack，首页双击退出）；Web+APK 2071 同批上线，记录链路 fixture 11/11 生产实测**；前批同日 十二.30 批次：v25.0.73 phase9 增长批次（25 新落地页，tools 17→42 页，sitemap 28→59 URL）；前批同日 十二.27~29 批次：v25.0.70 小众工具收尾（罗盘多门派Profile+立极尺专业盘+SEO五Cluster）/ v25.0.71 七政断语引擎六节+后台七项开关+用户活跃统计 / v25.0.72 七政四余学习资料+题库入易学学习区+中医正骨专区恢复上线（¥89付费+三态开关+后台改价SSOT）+支付链路两修复+Growth Pipeline cron 落地+APK 2070；前次 2026-08-31（十二.25 批次：百度 API 推送双站落地 14 URL+头条主站 ByteDanceVerify 补部署+每日自动推送队列 cron+主站 access_log 补缺；十二.24 批次：IndexNow 免登录主动推送双站 28 URL 202 受理+推送工具固化入仓；十二.23 批次：APK v25.0.67/2067 重建对齐线上 Web+统一分发源 latest.apk+升级提示/公告生效+主站 robots/sitemap 假文件修复；同日 SEO-GROWTH-ENGINE-SUPP-01 批次：程序化搜索增长引擎首批落地——13个C端差异化关键词静态落地页+1个B端行业方案页+robots/sitemap，v25.0.67 生产发布+公网验证 42/42 PASS+四端SSOT一致 add0f56；前批 2026-08-30 FINAL-OPERATIONS-COMPLETION-MASTER-05 最终封板：六模块后端+全量回归+公网 21/21）
...
| 项 | 值 |
|----|-----|
| 数据核实日期 | 2026-09-01（v25.0.74 批次核实，SSH+公网实测：version.json/records 页内容指纹/公网 chunks 四特征/升级接口 2071/记录链路 fixture 11 项/zhenggu 补交付 fixture B 8 项/PM2；前次同日 v25.0.73 批次） |
...
| 前端 Web 构建版本 | **v25.0.74**（buildId `v25.0.74_D20260901`，本地 Windows 构建（Turbopack）；本批：用户反馈双修复——排盘记录云端保存链路（clientStore syncRecordToBackend 正式链路+离线队列登录补传 flushPendingRecordSync+500KB 降级防护 _truncated）+13 工具页去 selectedClient 守卫+records 页 22 类型映射+摘要增强；含 v25.0.68~73 全部功能；公网 version.json 与公网 chunks 四特征已实测） |
| 后端发布版本 | v25.0.74（纯前端 JS 修复批次，零后端代码变更，运行时维持 `cf99433`；本批为验证 zhenggu 补交付 fixture Phase B 重启 PM2 一次（运行时不变），后端 API 与 v25.0.72 批次完全一致；前批 v25.0.73 纯静态、v25.0.72 academy 正骨门控+支付两修复全部在产） |
| Document Head | 本地=GitHub origin/main=服务器源码仓 `/root/yandaoguoxue-source` 三端 HEAD 一致同步（2026-09-01 v25.0.74 批次核实，git bundle 中转（`sync_v25_0_74.bundle` 传递 9afd6b1，用后即删）；提交链：`d9bde64`（MASTER-05 封板）→ `add0f56`（v25.0.67 SEO 首批）→ `d8f476a`（v25.0.69 四工具 SEO 集群页）→ `875ebfc`（v25.0.70 小众工具收尾）→ `5ece32b`（v25.0.71 断语引擎+活跃统计）→ `cf99433`（v25.0.72 七政学习资料+正骨专区+支付修复）→ `7f23c7c`（总账十二.27~29 回灌）→ `40630ae`（v25.0.73 phase9 增长批次）→ `193ffc7`（v25.0.73 部署脚本）→ `2b7d292`（总账十二.30 回灌）→ `9afd6b1`（v25.0.74 记录保存+返回手势修复，**生产构建源**）→ 文档回灌提交（总账十二.31+部署脚本+build.gradle 2071 对齐）；生产 Web current → v25.0.74 |
| SEO 落地页部署 | `https://yandaoguoxue.yandao.vip/{tools,learn,app,b}/*.html` 独立静态页（nginx try_files 直出，no-cache；独立于 APP，零后端逻辑改动；**v25.0.73 phase9 后：49 配置页+4 目录索引，tools 42 页，sitemap 59 URL（53 SEO+6 主站）**；配置唯一源 backend_deploy/seo/seoPagesConfig.json，生成器+质量门禁 **594 项**可复跑；Growth Pipeline 每日 03:20 cron 自动生成+门禁+公网内容指纹探测+推送（伪 Freshness 纪律：内容未变不重写，核查 PASS）；IndexNow 已推送 53 URL HTTP 200、百度队列 58 条（每日 09:10 cron 推 10 条自动消化）） |
| APK / versionCode | **25.0.74 / 2071**（`/api/public/app-version` 生产下发值；2026-09-01 重建：MainActivity OnBackPressedCallback 返回手势修复编入 dex（getOnBackPressedDispatcher 符号+「再按一次返回键退出言道国学」双击退出文案实测）+记录保存链路特征（flushPendingRecordSync/records/save）+records 页 22 类型标签+正骨/七政回归+五工具+五引擎+SEO 42 页烧录内置、统一分发目录 `/var/www/yandao.vip/app-download/latest.apk` 唯一文件（12,186,730 字节，MD5 536874a0e910dd3ce1a0f8b7f9f1ba02）、单一来源门禁 18/18 PASS、v25.0.74 升级公告已发布（含四条用户可读修复说明）） |
| 生产服务器 | 82.156.228.87（腾讯云轻量 北京，root） |
| 生产后端路径 | `/www/yandaoguoxue-backend`（PM2 `yandaoguoxue-backend`，端口 3001，online；本批 fixture Phase B 验证重启一次，运行时维持 `cf99433`） |
| 前端发布路径 | `/root/yandaoguoxue/releases/<tag>` + `/root/yandaoguoxue/current` 软链（nginx root 指向，current → v25.0.74；v25.0.73 保留可回滚） |
...
| 本轮生产验证 | **v25.0.74 全链验证 PASS**（Web：version.json v25.0.74/records 页 200+「我的排盘记录」指纹/公网 chunks 四特征实测（records/save+syncRecordToBackend+flushPendingRecordSync+_truncated，chunk 1a5o_brm_cs-5.js）/正骨+phase9 SEO 回归/记录链路 fixture 11 项生产实测；APK：aapt badging 2071/25.0.74+dex 返回手势符号+内容门禁+apksigner 验签+18 项单一来源门禁/升级接口+公告公网验证；zhenggu fixture 复验 A16/B8/CLEAN2）；前批 v25.0.73 公网 25 新页 h1 指纹全过 |
...
## 十二.23 APK v25.0.67 重建对齐 + 统一分发源 + 升级公告 + 主站 robots/sitemap 修复（2026-08-31）

**背景（项目方诉求）**：项目方反馈「手机 v25.0.60 无法检测到更新 / 网页版已是 v25.0.67 / 公告未更新 / APK 是否没在固定目录」。**SSH 实测确诊：v25.0.67 的 APK 从未构建**——全盘搜索仅两份 APK（源码仓 build 产物 + `/var/www/yandao.vip/app-download/latest.apk`），均为 08-26 的 v25.0.60/2059；`app-release-config.json` 维持 25.0.60/2059；公告仍是 v25.0.58 版本。手机检测不到更新的根因是服务器 `latestVersionCode(2059)` 未高于手机内置 2059，与「APK 放错目录」无关（统一目录一直在用，单一来源门禁 D20 此前 18 项全 PASS）。

**修复一：APK v25.0.67 重建（versionCode 2067）**：`backend_deploy/build_android_v25_0_67.sh`（模式复刻 build_android_v25_0_52.sh）——①资源源改为**直接复制生产 `releases/v25.0.67` 全量到源码仓 `out/`**（APK 内置资源与线上 current 逐字节一致，杜绝构建漂移）；②build.gradle 幂等对齐 2067/25.0.67（versionCode 递进规则核实：2054=v25.0.54…2059=v25.0.60，2000+minor 口径 → 2067）；③assets/public 同步+app-native.json 烧录 2067；④gradle assembleRelease 自动签名（Signer CN=Yandao O=DongguanYandaoTechnology 证书链与历史一致）；⑤APK 内容门禁：aapt badging 包名/versionCode/versionName + 内置 app-native.json 2067 + version.json v25.0.67 + **SEO 18 页内置齐全** + sitemap/robots + 差异化标题公式 + APK 唯一下载源 `app-download/latest.apk` + chunks 内下载链路关键词命中；⑥**原子部署**：先写 `.latest.apk.new` 再 `mv` 原子改名至 `/var/www/yandao.vip/app-download/latest.apk`，并清理目录内任何非 latest.apk 的 APK（单一来源纪律：分发目录仅此一个文件）；⑦随后才升 `app-release-config.json`（顺序纪律：先换包后改版本号，避免「提示升级却下到旧包」的升级循环）。

**修复二：升级提示+公告**：`/api/public/app-version` → 25.0.67/2067（downloadUrl 仍指向统一固定地址 latest.apk，downloadPage /friend）；`announcements.json` 以 v25.0.67 升级公告替换过时 v25.0.58 版本公告（a_v25_0_67_release，pinned/important，合伙人公告保留），AppUpgradeChecker 逻辑（服务器 versionCode > 壳内置 versionCode → 弹升级引导 → /friend → latest.apk）即恢复生效，**手机 v25.0.60 启动 3 秒内即弹升级提示**。两接口均为按请求实时读文件，无需 PM2 重启（零停机）。

**修复三：主站 robots.txt/sitemap.xml 假文件**：主站 www.yandao.vip 为 SPA fallback（try_files → index.html），`/robots.txt`、`/sitemap.xml` 请求返回 200 但内容是官网首页 HTML（与十二.22 验证文件同根因），平台提交 sitemap 必失败。修复：真实文件写入官网静态目录 `/www/yandao-company/`（nginx location / 的 try_files $uri 直接命中，**零 nginx 改动**）——robots.txt 声明 Sitemap；sitemap.xml 收录**实测 200 的 4 页**（/、/app.html、/app-pages/shenghuo.html、/app-pages/yandao.html；download/contact/feedback/privacy/terms 六页 nginx 有 location 但文件不存在实测 404，不入图）。内容级验证：robots→text/plain、sitemap→text/xml、apex 域名同步生效、官网首页回归 200。脚本+文件入仓 `backend_deploy/seo/main-site/`（fix_mainsite_seo.sh 幂等可复跑）。
...
## 十二.30 v25.0.73 phase9 增长批次——手机号/车牌号数字能量搜索集群+五小众集群 Guide 扩充（2026-09-01）

**定位**：DEV-SEO-GROWTH-ENGINE-PHASE9 执行批次，补充指令「搜索引擎流量扩展」。纯静态前端增量，零后端变更、零 APP 代码改动、零 PM2 重启（后端维持 `cf99433` 运行时）。

**工程内容（提交 `40630ae` 42 文件 = 26 新增+16 修改，生产构建源）**：
- **新增 25 个 SEO 落地页**（tools 17→42 页）：手机号数字能量集群 5 页（Core+4 Guide，cluster: phone）／车牌号数字能量集群 5 页（cluster: plate）／罗盘集群扩充 5 页（二十四山/三合/玄空/贞北罗盘等）／七政四余集群扩充 3 页（二十八宿/命宫/在线排盘）／立极尺+3、玄空+3、鲁班尺分位+1、户型定向+1。
- **sitemap.xml 28→59 URL**（53 SEO 页+6 主站页），robots.txt 同步；tools/ 目录索引页刷新为 42 页清单。
- **合规纪律**：全部新页以「传统文化学习/娱乐参考」口径撰写，不宣传科学预测能力，不写确定性吉凶结论（产品定位纪律第七章）。
- **Growth Pipeline 全流程手动执行**（phase9 上线后）：生成→质量门禁 **594 项 PASS/0 FAIL**→公网内容指纹探测 53/53→IndexNow 推送 53 URL 全部 HTTP 200→百度队列追加 25 条（33→58 条，每日 09:10 cron 推 10 条自动消化）。

**部署与验证（全流程纪律）**：本地构建 v25.0.73（buildId `v25.0.73_D20260901`）→ git bundle 中转（`sync_v25_0_73.bundle`，用后即删）同步服务器源码仓 HEAD=`193ffc7` → 服务器 Node v22 构建（产物核查：version.json 烧录+tools 42 页+25 新页齐全+目录索引）→ tar 解包至 releases/v25.0.73 → 内容门禁（版本烧录+h1 指纹抽查 4 页+差异化合规+主站回归+IP 零泄漏）→ **current 原子切流 → releases/v25.0.73** → 公网验证 25/25 新页 h1 内容指纹（curl 内容级实测，非 200 状态码——防 SPA 兜底假阳性）+11/11 修改页回归+sitemap 59 URL+工具索引+API health。

**收尾核查与服务器清理（2026-09-01 18:5x CST，SSH 实测）**：
- **历史 bundle 清理**：`sync_v25_0_72.bundle`/`sync_v25_0_73.bundle` 均已在部署流程中删除（/root 下已无 sync bundle）；遗留的两个 8/23 传输媒介 `v22.bundle`/`v22fix.bundle` 经 `git bundle list-heads`+`git cat-file -t` 双重验证 HEAD 提交（c2e3741/d1eaf27）均在服务器 git 历史中且无脚本/cron 引用，已删除。
- **旧构建 tar 清理**：out_v25_0_67~72 共 5 个已解包 tar 删除（约 30MB），保留最新 out_v25_0_73.tar.gz 作便携回滚源；releases/ 目录 438MB（15 版本），磁盘 16G/50G（31%）充裕，旧版本目录暂留待确认无需远程回滚后清理。
- **服务器源码仓 git 状态治理**：`public/version.json` 服务器构建再生成的 builtAt 时间戳差异（版本号一致）已 `git checkout` 恢复；`backend_deploy/seo/baidu_push_queue.txt` 为**有意的 symlink 设计**（→ `/root/backend-auth/data/baidu_push_queue.txt`，58 条权威队列数据持久化于后端数据目录，源码仓 rebuild 不丢队列），git status 长期显示的 `T`（typechange）标记为预期运行时状态——**本地仓不再跟踪该文件内容变更**（队列数据所有权归服务器 cron），规避下次 bundle pull 冲突；若未来部署脚本需 pull 含该文件变更的 bundle，须先丢弃服务器侧 staged 变更再恢复 symlink。
- **生产终验**：current → releases/v25.0.73（readlink 实测）；公网 `https://yandaoguoxue.yandao.vip/version.json` = buildId `v25.0.73_D20260901`；phase9 新页 h1 指纹（手机号码数字能量分析等）公网命中；PM2 yandaoguoxue-backend online 未重启；三端 HEAD 一致（本地=GitHub=服务器源码仓 `193ffc7`+文档回灌提交）。

## 十二.31 v25.0.74 用户反馈双修复——排盘记录云端保存链路+安卓返回手势（2026-09-01）

**定位**：用户反馈两 bug 修复批次：①「排盘工具排盘记录不能保存」②「安卓用户屏幕右滑往屏幕中间不能返回上一级」。纯前端+原生壳修复（提交 `9afd6b1` 20 文件，生产构建源），零后端代码变更（运行时维持 `cf99433`）。

**根因分析**：
- **记录不保存（双根因）**：其一，`src/lib/clientStore.ts` 云端同步调用死路径 `/api/records`（后端从未有此路由，全部 404），记录只落 localStorage——换设备/重装即丢，且 `/records` 页读后端 API 导致「保存了也不显示」；其二，13 个工具页（bazi/ziwei/qimen/daliuren/liuyao/meihua/taiyi-sanshi/name/carplate/phone/zeri/xuankong-feixing/qizheng）存在 selectedClient 守卫——未选择客户档案时直接跳过保存。
- **安卓返回失效**：Capacitor 8 BridgeActivity 不处理返回键（@capacitor/app 插件亦未安装），系统返回手势（右边缘向中间滑）与 KEYCODE_BACK 直接走 Activity 默认 finish() 退出 APP，WebView 历史栈（Next.js router 产生的 history）从未被消费。

**工程内容（提交 `9afd6b1`）**：
- **clientStore.ts（+50 行）**：asyncSyncCreateRecord 改走 syncRecordToBackend（POST /api/auth/records/save 正式 JWT 链路）；离线/失败入队 addPendingOp，登录成功（loginService.ts 挂钩）触发 flushPendingRecordSync 补传；makeBackendPayload 500KB 降级防护（超限精简 payload+`_truncated` 标记+inputParams 保留，防离线队列永久积压）。
- **13 工具页去守卫**：selectedClient 守卫移除（clientId 允许空串），未选客户直接排盘也保存记录。
- **records/page.tsx（+41 行）**：RECORD_TYPE_MAP 扩至 22 种类型全集（姓名/起名/八字/紫微/大六壬/六爻/梅花/奇门/七政四余/罗盘/小六壬/合婚/黄历/太乙三式/玄空飞星/立极尺/鲁班尺/号码/车牌/择日/解梦/体质测评）+修正 liuren 键名；getRecordSummary 摘要增强（排盘时间/号码/车牌/梦境等关键字段直显）。
- **MainActivity.java（+42 行）**：注册 OnBackPressedCallback 接管返回（WebView canGoBack→goBack()，Next.js 响应 popstate 返回上一级、弹窗先关；首页无历史→双击退出防误触+Toast 提示）；同时兼容传统 KEYCODE_BACK 与 Android 13+ 手势两种派发路径；保留 v25.0.55 下载监听逻辑。

**部署与验证（全流程纪律）**：
- **Web v25.0.74**：本地构建（buildId `v25.0.74_D20260901`，产物 1924 文件）→ tar 上传 → 内容门禁（版本烧录+记录链路特征 flushPendingRecordSync×8 chunks/records/save×1/类型映射×5+records 页「我的排盘记录」指纹+主站/正骨/七政回归+tools 42 页+sitemap 59 URL+IP 零泄漏全过）→ **current 原子切流 → releases/v25.0.74** → 公网热路径立检（version.json v25.0.74、/records/ 200+内容指纹、公网 chunks 四特征实测：records/save+syncRecordToBackend+flushPendingRecordSync+_truncated 全部命中，records 页 chunk 369j942p2s0z-.js/clientStore chunk 1a5o_brm_cs-5.js）。
- **构建一致性核查**：上线时发现服务器 current 曾被切流到一个 builtAt 15:53 的中间构建（上下文丢失前遗留），经 chunks 特征计数+records 页大小（26739 vs 26513 字节）+内容归一化 diff 三重比对确认其缺最终源码的 records 摘要增强等尾批修改，已用最终源码构建（16:41 完成于全部源码修改之后）重新解包切流覆盖，公网 chunk 引用实测对齐。
- **已知误报说明**：deploy 脚本热路径检查 `/records`（无斜杠）为 nginx 301 目录重定向，curl 未加 `-L` 时误报失败——`/records/` 直访 200+内容指纹正常，非缺陷。
- **APK 25.0.74 / versionCode 2071**（build_android_v25_0_74.sh）：MainActivity 源码同步服务器源码仓 → gradle assembleRelease → APK 内容门禁全过（aapt badging 2071/25.0.74；**dex 返回手势符号：getOnBackPressedDispatcher 命中+「再按一次返回键退出言道国学」文案命中**；记录保存链路特征内置；records 页 22 类型标签；正骨专区+七政学习链接回归；五工具页；五引擎烧录；SEO 42 页；apksigner 验签 CN=Yandao）→ 统一分发 `/var/www/yandao.vip/app-download/latest.apk` 唯一文件（12,186,730 字节，MD5 `536874a0e910dd3ce1a0f8b7f9f1ba02`，公网与服务器文件 MD5 一致）→ app-release-config.json 升 2071 → v25.0.74 升级公告（四条用户可读修复说明）→ **单一来源门禁 D20 18/18 PASS**。

**Fixture 验证（生产服务器实测）**：
- **记录链路 records_fixture_v25_0_74.js 11/11 PASS**：保存 success+record_id 返回/type=bazi 筛选可查+record_data 对象往返（inputParams 完整）/全量列表含新记录/无 token 401 拦截/空 record_type 400/**超 500KB payload 400 拒绝**/删除 success+列表不再含/DB fixture 记录清零无残留。
- **zhenggu fixture 补交付复验（上批遗留 pending 项）**：Phase A 复跑 16/16（价格 SSOT 89 元裁决/未解锁 402 Paywall/权益放行/三态开关）→ PM2 重启 → **Phase B 8/8（重启后权益为空→query 返回 PAID→补交付写入 user_entitlements 永久权益→关联回源订单→access unlocked=true→benefit_delivered=1 幂等→重复 query 不重复交付）** → CLEAN 2/2（矩阵默认 ON/权益清除）。
