...
> 最后更新: 2026-08-24（生产版本 v25.0.47_22 + APK v25.0.52：MARKETING-POSTER-V2-AI 邀请裂变海报营销化升级+AI智能文案生成——①三套社交裂变模板全量重构（朋友圈种草版「私藏很久的国学宝藏工具，终于舍得分享了」4条结果型卖点+🎁福利行动召唤条/社群引流版「免费！专业级国学工具平台」【易学工具】【中医学习】两栏分组6条卖点/学习进阶版「你的随身国学学习助手」📚🎯📝💡价值点）②渲染引擎视觉规范（主标题≤宽1/8加粗自适应、二维码≥宽1/4、CTA行动召唤上置、合规小字#AAAAAA、dense紧凑排版防重叠）③海报比例 3:4 竖版 1080×1440+三模板底色统一米色国风④AI智能文案生成（aiCopy.ts：营销提示词3风格并行生成[朋友种草/专业干货/简洁直接]+敏感词过滤+3套内置兜底；「✨AI换文案」一键应用实时重渲染+「再来一组」+「恢复模板文案」）⑤分享文案库三场景（朋友圈种草/社群引流/私聊好友+一键复制）；公网 E2E 17 PASS/0 FAIL，APP 经 Capacitor server.url 直载线上页面无需重建 APK；前一版本 v25.0.47_21：FIX-V21 四大核心闭环（会员支付数字userId根因修复真实账号E2E实测/版本号单一数据源占位符注入四处一致/后台订单中心交易号+筛选导出/缓存30秒轮询悬浮提示点击即清）；指令 MARKETING-POSTER-V2-AI）
...
| 当前生产版本 | **v25.0.47_22**（buildId v25.0.47_22_D20260824，2026-08-24 发布：MARKETING-POSTER-V2-AI 海报营销化升级+AI智能文案生成——①**三套社交裂变模板重构**：朋友圈种草版（默认，主标题「私藏很久的国学宝藏工具，终于舍得分享了」+副标题两行+4条✅结果型卖点+🎁福利行动召唤条「扫码注册即得免费AI解析次数/每日签到领积分兑权益」）/社群引流版（「免费！专业级国学工具平台」+【易学工具】【中医学习】两栏分组6条▪卖点+「永久免费基础功能·无强制广告」标注）/学习进阶版（「你的随身国学学习助手」+📚🎯📝💡4条学习价值点+「适合爱好者/从业者/学生党」人群标注）；②**渲染引擎视觉规范**：主标题字号≤海报宽度1/8加粗自适应两行+视觉重心靠上1/3、二维码尺寸≥海报宽度1/4四周留白、CTA行动召唤上置二维码上方、行动召唤条浅底福利条、合规小字#AAAAAA置最底、dense紧凑排版（h/w≤1.4自动压缩间距防福利条压合规底栏重叠）；③**海报比例3:4竖版**（1080×1440，renderViralPoster+buildViralRecs两处由R9_16改R3_4）+三模板底色统一米色国风（社群版米色赭红T02/学习版米色墨绿T05）；④**AI智能文案生成**（aiCopy.ts：内置营销提示词3风格并行生成[朋友种草风/专业干货风/简洁直接风]+行式解析+敏感词过滤逐套丢弃+3套内置兜底；邀请页+AI推广助手页「✨AI换文案」按钮+底部弹层3风格卡片预览+一键应用实时重渲染+「再来一组」无限刷新+「恢复模板文案」；系统分享优先AI配文）；⑤**分享文案库三场景**（朋友圈种草长文案/社群引流短文案/私聊好友福利文案+一键复制）；公网E2E实测 17 PASS/0 FAIL（真实账号13612674128：登录/三模板切换/海报3:4尺寸断言/AI换文案生成应用恢复/保存下载/三场景文案库）；APP经Capacitor server.url直载线上页面，v22网页部署即覆盖APP端无需重建APK |
| Git HEAD | v25.0.47_22 代码提交 d1eaf27（main，c2e3741 主功能+d1eaf27 视觉规范修复两提交+总账收口提交；本地=GitHub=服务器源码仓=生产运行目录 四端一致；海报营销化升级+AI智能文案生成+发布脚本+E2E 测试脚本） |
...
| 前端发布 | /root/yandaoguoxue/releases/&lt;tag&gt; + current 软链（SPA 静态导出）；当前 current→v25.0.47_22，回滚目标 v25.0.47_21；目录现状 _5/_6/_8/_9/_17/_21/_22 等多版（磁盘充足暂留，如需清理保留 _21+_22 即可） |
