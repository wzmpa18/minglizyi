...
> 最后更新: 2026-08-23（生产版本 v25.0.47_17 + APK v25.0.49：四大修复上线——①后台导航全端统一抽屉式（删除桌面端常驻侧栏，默认收起+汉堡唤出+遮罩覆盖，内容区全宽不遮挡）②商业中心会员购买跳转修复（会员中心 href 原生锚点直达不依赖 JS+商业中心默认展开）③版本号动态化（页脚/关于弹窗读 version.json+系统中心「检查更新」手动升级按钮）④中医典籍混元 AI 全量导入（177 章：6 部典籍章节补全+新增濒湖脉学/药性赋/汤头歌诀/千金要方/医宗金鉴/中藏经 6 部，classicsExtra 外挂模块架构）；APK versionCode 2048→2049 触发存量用户升级提醒；指令 FIX-V17-DRAWER-MEMBER-UPDATE-CLASSICS）
...
| 当前生产版本 | **v25.0.47_17**（buildId v25.0.47_17_D20260823，2026-08-23 发布：FIX-V17 四大修复——①**后台全端统一抽屉导航**：删除 v16 的 isDesktop 桌面常驻侧栏方案（该方案内容区仍被侧栏挤压），全端统一为抽屉覆盖式（默认收起+顶部汉堡「打开导航菜单」唤出+遮罩层 zIndex:99+translateX 滑入滑出+内容区全宽 100vh 不避让），桌面/移动体验一致；②**商业中心会员购买跳转修复**：profile 页 ZoneItem 组件新增 href 参数支持原生 `<a>` 锚点模式，会员中心入口 href="/membership/" 原生跳转（不依赖 router.push/JS，任何 JS 异常下均可点击），商业中心 Zone defaultOpen 默认展开解决入口藏太深；③**版本号动态化+手动检查更新**：IcpFooter 页脚与关于弹窗版本号改 fetch /version.json 动态读取（替换硬编码 v25.0，fetch 失败兜底原文案），系统中心新增「检查更新」入口（handleCheckUpdate 手动拉 version.json 对比本地 appVersion，发现新版本显示「新版本」角标+点击刷新即升级，配合 v16 AppUpgradeChecker 自动检测形成双保险）；④**中医典籍混元 AI 全量导入**：服务器 tcm_gen 混元批量生成管道产出 12 部典籍 177 章（素问 24/灵枢 16/金匮 16/温病 12/难经 25/本草 9 章补全+新增濒湖脉学 27/药性赋 5/汤头歌诀 15/千金要方 10/医宗金鉴 8/中藏经 10 章），classicsExtra.ts 外挂模块（CHAPTER_APPENDS 现有典籍补全+EXTRA_BOOKS 新增典籍），classics.ts buildMergedBooks() 自动合并（getAllBooks/getBookById/getChapterById/searchClassics 四函数全走合并数据）；关键工程细节：书 ID 映射（生成文件 jingui/shennong→classics.ts 实际 jinkui/bencao）+章节序号自动接续现有最大值防 ID 冲突；⑤**APK v25.0.49**（versionCode 2048→2049/versionName 25.0.49）：内置 v25.0.47_17 资源，app-release-config.json 更新至 2049 触发存量 v25.0.48 用户升级弹窗） |
| Git HEAD | v25.0.47_17 代码提交 f88b12a（main，本地=GitHub=服务器源码仓=生产运行目录 四端一致；全端抽屉+会员入口+检查更新+典籍AI全量导入，10 文件 +2077/-48） |
| 正式 APK | https://yandaoguoxue.yandao.vip/app-download/yandao-guoxue-v25.0.49-release.apk（v25.0.49 / versionCode 2049，v2 签名，内置 v25.0.47_17 资源 11.76MB）；服务器另挂别名文件 guoxue-chuancheng-v1.0-release.apk（同包，兼容存量分享海报/链接）；旧 v25.0.48/v25.0.47 包保留在分发目录作回滚备份；升级提示数据源 /www/yandaoguoxue-backend/data/app-release-config.json（latestVersionCode 2049） |
...
| 前端发布 | /root/yandaoguoxue/releases/&lt;tag&gt; + current 软链（SPA 静态导出）；当前 current→v25.0.47_17，回滚目标 v25.0.47_16；目录现状 _5/_6/_8/_9/_10/_12/_13/_14/_15/_16/_17 十一版（磁盘充足暂留，如需清理保留 _16+_17 即可） |
...
| ADMIN_CONTROL_CENTER（统一后台） | VERIFIED（v25.0.47_17 全端统一抽屉导航） | /admin 全端统一抽屉式导航（v17 删除桌面常驻侧栏方案：默认收起+汉堡「打开导航菜单」唤出+遮罩覆盖，内容区全宽零避让，桌面/移动一致）；adminRoles.js 统一权限模块：SUPER_ADMIN/FINANCE_ADMIN/OPERATOR_ADMIN 服务端强校验（越权 403+审计 AUDIT_BLOCK_*），子密钥 SHA256 哈希存储；公网实测财务密钥越权 5 项全 403、运营密钥越权财务 403；密钥管理页 /admin/keys 签发/禁用子密钥 |
...
- 当前版本 v25.0.49（versionCode 2049，2026-08-23 v25.0.47_17 重建）；内置 v25.0.47_17 资源（含升级提示体系+典籍 177 章+全端抽屉导航）；服务器构建（Gradle 8.9 本地发行版+Android SDK 重装后）
- 升级链路：AppUpgradeChecker 启动/回前台检测 versionCode 落后即弹升级窗（app-release-config.json 为 SSOT，latestVersionCode 2049）
- 分发：/var/www/yandao.vip/app-download/yandao-guoxue-v25.0.49-release.apk + 别名 guoxue-chuancheng-v1.0-release.apk（双文件，存量分享链路兼容）；旧 v25.0.48/v25.0.47 包保留回滚
