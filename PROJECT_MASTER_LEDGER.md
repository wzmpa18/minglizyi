...
> 最后更新: 2026-08-24（生产版本 v25.0.47_21 + APK v25.0.52：FIX-V21-PAY-VERSION-CACHE-FINAL 四大核心闭环——①会员支付链路修复（旧登录态数字userId根因：auth.ts normalizeLegacyProfile+paymentService 强制 String 统一口径；支付失败弹窗明确报错；未登录弹登录引导+登录回跳 autopay 自动唤起支付，真实账号 13612674128 E2E 全流程实测二维码弹出+订单落库）②版本号单一数据源（公告 {APP_VERSION}/{WEB_VERSION} 占位符实时注入、检查更新点击即清缓存刷新、后台总览 Web+APP 双版本、四处版本完全一致）③后台订单中心（transaction_id 微信交易号持久化+手机号/昵称/邀请人/返佣明细+状态与日期区间筛选+CSV 导出+权限仅 SUPER_ADMIN/FINANCE_ADMIN，仪表盘订单卡片点击跳转明细）④缓存机制升级（VersionChecker 30 秒轮询+悬浮更新提示+点击清 CacheStorage/注销 SW 强刷+更新完成提示）；前一版本 v18~v20：典籍搜索高对比+购买登录引导+网页自动刷新（_18）、官方公告栏+APK 链接统一 latest.apk（_19）、首页死键清理+四柱白底红字+更新自动清缓存+抽屉容器定宽修复（_20）；指令 FIX-V21-PAY-VERSION-CACHE-FINAL）
...
| 当前生产版本 | **v25.0.47_21**（buildId v25.0.47_21_D20260823，2026-08-23 发布：FIX-V21 四大核心闭环——①**会员支付链路修复**：根因=旧登录态 localStorage 中 userId 为数字，后端 /api/payment/create 要求字符串（length≥4）返回「用户ID无效」；修复=auth.ts normalizeLegacyProfile（getLoginState/getUserProfile 读取时数字→字符串）+paymentService getCurrentUserId 强制 String(uid)+后端口径对齐；会员页新增支付失败弹窗（payError 状态，禁止静默死键）+未登录点击开通弹登录引导（sessionStorage yandao_membership_autopay 保留档位+yandao_login_redirect 回跳 /membership?autopay=1，登录成功自动唤起支付）；真实账号 13612674128 E2E 实测：未登录引导弹窗→登录→月度档→立即开通→微信付款二维码弹出→订单落库全 PASS；②**版本号单一数据源**：announcementRoutes.js 版本占位符注入（{APP_VERSION}←app-release-config.json latestVersion、{WEB_VERSION}←current/version.json，公告文本实时替换永不落后）；个人中心检查更新 handleCheckUpdate 发现新版本直接 reloadWithCachePurge 强刷（写 yandao_updated_to 更新完成提示）；后台总览 version+appVersion 双版本显示；③**后台订单中心**：paymentRoutes.js user_orders 表新增 transaction_id 列（ALTER TABLE 增量迁移+INSERT 持久化微信交易号）；adminUnifiedRoutes.js /orders 增强（LEFT JOIN users 取手机号/昵称+邀请人昵称/手机号+返佣状态+状态/日期区间筛选/分页）+/orders/export CSV 导出（UTF-8 BOM Excel 直开+审计留痕）；权限 adminAuthUnified('FINANCE_ADMIN','finance') 仅超管/财务可见，运营 403；admin/orders/page.tsx 订单列表页（11 列表格+日期筛选+导出按钮+分页）；仪表盘订单卡片 onClick 跳转 /admin/orders?status=PAID；④**缓存机制升级**：VersionChecker.tsx 30 秒轮询 version.json+页面可见立即检查+发现新版本悬浮提示「发现新版本，点击立即更新」+点击清 CacheStorage+注销 SW+强刷一步完成+更新完成绿色提示 |
| Git HEAD | v25.0.47_21 代码提交 527914a（main，本地=GitHub=服务器源码仓=生产运行目录 四端一致；支付/版本/订单/缓存四大闭环+发布脚本+E2E 测试脚本） |
...
| 前端发布 | /root/yandaoguoxue/releases/&lt;tag&gt; + current 软链（SPA 静态导出）；当前 current→v25.0.47_21，回滚目标 v25.0.47_20；目录现状 _5/_6/_8/_9/_17/_21 等多版（磁盘充足暂留，如需清理保留 _20+_21 即可） |
