...
> 最后更新: 2026-08-25（生产版本 v25.0.47_28：FIX-V28-LEGACY-SHELL-RESCUE 老壳救援——用户二次报告「手机APP还是不能更新，点击更新按键没有用」根因确诊：用户手机为 server.url 老壳 APK（v25.0.47_3 及更早，versionCode≤2047，WebView 直载线上页面、本地无 app-native.json），该类壳页面内容跟随线上更新但 APK 本体永远停留老版本，且一切升级检测对其失效——检查更新误报「已是最新」（/version.json 线上==线上恒等）、升级弹窗永不触发；新增 src/lib/nativeDetect.ts 统一三通道探测根治：①内置资源壳（≥2048）app-native.json 精确比对 versionCode；②老壳由 window.Capacitor.isNativePlatform() 桥判定（任何时代壳内必注入/浏览器必无，唯一可靠判据）→ 判定必然过旧，Toast「检测到 APP 版本过旧」+自动跳转下载页 latest.apk 覆盖安装一步转正；③浏览器保留 version.json 对比清缓存刷新；AppUpgradeChecker 启动3s+回前台自动弹窗同步接入老壳救援；纯 web 发版（老壳直载线上即获救援，无需新 APK，2054 仍为最新）；单一分发源复核：全站下载入口唯一指向 /app-download/latest.apk；前一版本 v25.0.47_27：FIX-V27-CHECK-UPDATE 检查更新双通道 + APK v25.0.54(2054)）
...
| 当前生产版本 | **v25.0.47_28**（buildId v25.0.47_28_D20260825，2026-08-25 发布，FIX-V28-LEGACY-SHELL-RESCUE：**server.url 老壳救援——检查更新「点击没用」二次报告根治**——①**根因确诊**（用户截图证据链：页脚 v25.0.47_27 + 检查更新子文案「当前已是最新版本 v25.0.47_27」为网页通道格式，而 v27 原生通道应显示 APK 版本号 25.0.54）→ 用户手机为 v25.0.47_3 及更早的 server.url 老壳（versionCode≤2047，capacitor.config server.url 直载线上页面）：该壳本地无 app-native.json → v27 双通道的探测文件 404 落入网页通道 → /version.json 壳内走线上与运行版本恒等 → 永远「已是最新」；AppUpgradeChecker 同因静默退出 → 升级弹窗永不触发 → APK 本体永久停留老版本（页面内容却一直跟随线上更新，极具迷惑性）；②**修复**——新增 src/lib/nativeDetect.ts 统一探测三通道：detectNativeShell() 优先 fetch /app-native.json（内置资源壳≥2048 精确 versionCode）→ 404 时 window.Capacitor.isNativePlatform() 桥判定（Capacitor 桥在任何时代 APK 壳内必注入、浏览器必不存在，是唯一可靠判据；项目未装 @capacitor/app/device 插件故用桥存在性而非插件取版本）→ 桥存在即老壳（versionCode 未知但必然≤2047）→ LEGACY_SHELL_MAX_CODE=2047 判定必然过时；handleCheckUpdate 三通道化：老壳点击 → Toast「检测到 APP 版本过旧，正在打开下载页，请下载安装 v25.0.54…」+900ms 跳 downloadPage 覆盖安装一步转正为内置资源壳；AppUpgradeChecker 同步接入：老壳启动 3s/回前台自动弹「发现新版本」窗（副标题「检测到旧版 APP，最新版本 v25.0.54」）→ 老板存量老壳手机打开 APP 即收到引导；③**单一分发源复核**（用户指令「所有下载APP全部指向一个地方」）：全站唯一 APK 地址 https://yandaoguoxue.yandao.vip/app-download/latest.apk——/friend、/download 落地页运行时动态 downloadUrl+APK_URL_FALLBACK、检查更新、AppUpgradeChecker、app-release-config.json 全部指向；发新版本仅需替换 latest.apk+更新 app-release-config（构建脚本全自动三文件分发：正式包名+guoxue-chuancheng 别名+latest.apk 同 MD5）；www.yandao.vip 无独立官网 HTML（目录仅 app-download 分发）；④**交付链**：纯 web 发版（老壳直载线上即获救援；内置壳 2048+原生通道不受影响；2054 仍为最新 APK 无需重建）→ 本地 commit 27b1bc7 → GitHub push → bundle 同步服务器源码仓 → build.sh 构建 46s（特征门禁：老壳救援文案 1 chunk/isNativePlatform 8/2047 8 全入包）→ releases/v25.0.47_28 发布 → **切流纠偏**（首次误切 releases/current 软链无效，诊断发现 nginx root=/root/yandaoguoxue/current，正确 ln -sfn 切换后生效）→ 公网 version.json v28+/profile chunks 救援代码线上命中+5 页面 200+latest.apk MD5 665dfc0c 一致；前一版本 v25.0.47_27 详见版本演进表） |
| Git HEAD | 27b1bc7（main，本地=GitHub=服务器源码仓=生产运行目录 四端一致；v28 提交 27b1bc7 老壳救援；v27 提交 4acfe0f 检查更新双通道+APK 2054；v26 提交链 783f517→c68b899→692ca0c→478a613；1b7046b v27 总账收口） |
| 正式 APK | https://yandaoguoxue.yandao.vip/app-download/yandao-guoxue-v25.0.54-release.apk（v25.0.54 / versionCode 2054，v2 签名 CN=Yandao/O=DongguanYandaoTechnology，内置 v25.0.47_27 资源+双侧侧滑返回+检查更新修复）；别名文件 guoxue-chuancheng-v1.0-release.apk 与 latest.apk 同包（三文件 MD5 一致 665dfc0c639f72f3b9eb2dcfc3db8691）；历史包 v25.0.47~v25.0.53 保留分发目录作回滚；升级提示数据源 /www/yandaoguoxue-backend/data/app-release-config.json（已指 2054）；云端构建 codemagic.yaml android-workflow（ANDROID_KEYSTORE_B64 加密变量，值见核心文档《06_Codemagic_ANDROID_KEYSTORE_B64.txt》）；本地归档 言道国学_核心文档/APK安装包/言道国学_v25.0.54_2054_离线完整包.apk |
...
| 前端发布 | /root/yandaoguoxue/releases/&lt;tag&gt; + **/root/yandaoguoxue/current 软链（nginx root 真实指向，v28 切流纠偏时确认；releases/current 软链为冗余同指）**；当前 current→v25.0.47_28，回滚目标 v25.0.47_27；目录现状 _5/_6/_8/_9/_17/_21/_22/_23/_24/_25/_26/_27/_28 等多版（磁盘充足暂留，如需清理保留 _27+_28 即可） |
