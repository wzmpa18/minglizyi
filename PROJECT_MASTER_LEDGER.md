...
> 最后更新: 2026-08-25（生产版本 v25.0.47_27 + APK v25.0.54：FIX-V27-CHECK-UPDATE 「检查更新」按钮点击无反应根治——APK 原生壳内旧逻辑 fetch /version.json 走本地内置文件（native-api-patch 只改写 /api/）永远比出相同版本零反馈；重写 handleCheckUpdate 双通道：①原生壳（探测 app-native.json）改拉服务器 /api/public/app-version 比对 versionCode，有新版 Toast+700ms 自动跳转下载落地页一键安装，已是最新 Toast「当前已是最新版本vXX」，接口/网络异常明确报错；②网页版保留 version.json 对比，有新版清缓存刷新一步完成；全分支接入全站 showToast 反馈；APK v25.0.54(versionCode 2054) 三文件发布+升级配置 2054+公告栏 v25.0.54 更新公告置顶；前一版本 v25.0.47_26：DEV-V22-PARTNER-V2-BACKSWIPE 合伙人渠道体系V2 + APK 双侧侧滑返回 + APK v25.0.53(2053)）
...
| 当前生产版本 | **v25.0.47_27**（buildId v25.0.47_27_D20260825，2026-08-25 发布，FIX-V27-CHECK-UPDATE：**「检查更新」点击无反应根治**——①根因：APK 原生壳内旧 handleCheckUpdate fetch `/version.json` 走本地内置资源（native-api-patch 仅改写 `/api/` 前缀），本地 buildId 恒等于运行 buildId，永远判定「已是最新」且无任何 UI 反馈，用户感知「点了没反应、不能更新」；②修复（src/app/profile/page.tsx 重写 handleCheckUpdate 双通道）：原生壳探测 `/app-native.json`（仅 APK 内置存在，网页 404 自动跳过）→ 拉服务器 `/api/public/app-version` 比对 versionCode——有新版：Toast「发现新版本 vX，正在打开下载页…」+700ms 后 location.href 跳转 downloadPage（/friend 落地页自动下载 latest.apk）一键安装；已是最新：Toast「当前已是最新版本 vX」；接口异常/网络异常：明确报错提示；网页版通道保留 version.json 对比，有新版 Toast+reloadWithCachePurge 一步完成；全部分支接入全站 showToast（components/ui/Toast）确保点击必有可见反馈；③版本号同步：web v25.0.47_27（package.json+public/version.json）/ APK v25.0.54 versionCode 2054（build.gradle）；④交付链：服务器 bundle 同步 4acfe0f→build.sh 构建（产物含「正在打开下载页」「当前已是最新版本」特征各 1 chunk）→releases/v25.0.47_27 发布 current 切流→APK 全门禁构建（六项关键代码检查含新特征+签名验证 CN=Yandao）→三文件分发+app-release-config 2054+公网三直链 200+MIME+MD5 一致；⑤公告栏更新：置顶公告改 v25.0.54 内容（{APP_VERSION} 占位符自动渲染）+旧 v25.0.51 公告已取消置顶；前一版本 v25.0.47_26 详见版本演进表） |
| Git HEAD | 4acfe0f（main，本地=GitHub=服务器源码仓=生产运行目录 四端一致；v27 提交 4acfe0f 检查更新热修；v26 提交链 783f517 合伙人V2+侧滑返回 → c68b899 E2E修复收口 → 692ca0c Codemagic云构建收口+总账收口 → 478a613 v25.0.53 APK构建脚本收口） |
...
| 前端发布 | /root/yandaoguoxue/releases/&lt;tag&gt; + current 软链（SPA 静态导出）；当前 current→v25.0.47_27，回滚目标 v25.0.47_26；目录现状 _5/_6/_8/_9/_17/_21/_22/_23/_24/_25/_26/_27 等多版（磁盘充足暂留，如需清理保留 _26+_27 即可） |
