...
> 最后更新: 2026-08-24（生产版本 v25.0.47_24 + APK v25.0.52：ADMIN-USER-INFO-ZOOM-OFF 后台用户完整注册信息+屏幕放大默认关闭——①后台「用户管理」(/admin/moderation 用户 tab) 表格新增**完整手机号列+注册邮箱列**（后端 /moderation/users 去脱敏直出：原 slice(0,3)+'****'+slice(-4) 改为完整返回，SELECT 补 email 字段，后台已鉴权 SUPPORT_ADMIN/ops；搜索支持昵称/用户ID/手机号/邮箱四字段）；②个人中心-通用设置「屏幕放大」**默认关闭**（GlobalZoomProvider 挂载逻辑翻转：yandao_zoom_disabled 未设置时默认 zoomDisabled=true，仅显式开启 ==="0" 才启用缩放并恢复记忆级别；首次捏拉提示仅已开启时展示；设置页 zoomEnabled 初始 false；历史用户曾开="0"保持开/曾关="1"保持关不受影响）；③本版含后端变更（adminUnifiedRoutes.js 同步 /www/yandaoguoxue-backend + PM2 重启）；公网 E2E 实测 16 PASS/0 FAIL（后台表格列/完整号码断言 18 条 0 脱敏/手机号搜索命中 13612674128/无痕默认关灰底/开关切换持久化/无首次提示）；前一版本 v25.0.47_23：FIX-V23-MEMBER-BUY-GUIDE 会员购买引导 fixed 悬浮底栏（任何滚动位置即见「立即开通」+点击卡片文案联动+二维码弹出，E2E 17 PASS）；指令 ADMIN-USER-INFO-ZOOM-OFF）
...
| 当前生产版本 | **v25.0.47_24**（buildId v25.0.47_24_D20260824，2026-08-24 发布：ADMIN-USER-INFO-ZOOM-OFF 后台用户完整注册信息+屏幕放大默认关闭——①**后台用户管理完整注册信息**：/admin/moderation 用户 tab 表格新增「手机号」「邮箱」两列，后端 /api/admin/unified/moderation/users 去脱敏直出（原 slice(0,3)+'****'+slice(-4) 脱敏改为完整返回，SELECT 补 email 字段），搜索框支持昵称/用户ID/手机号/邮箱四字段（非数字查询走 nickname/phone/email 三字段 LIKE）；②**屏幕放大默认关闭**：GlobalZoomProvider 挂载逻辑翻转——yandao_zoom_disabled 未设置（新用户/无痕）默认 zoomDisabled=true 禁用全部缩放交互（双指捏拉/双击/Ctrl+滚轮），仅显式开启（==="0"）才启用并恢复记忆缩放级别；首次「双指捏拉可放大页面」提示仅已开启时展示；设置页 zoomEnabled 初始 false；历史用户不受影响（曾开="0"保持开/曾关="1"保持关）；③**本版含后端变更**：adminUnifiedRoutes.js 同步 /www/yandaoguoxue-backend + PM2 restart yandaoguoxue-backend + 健康检查；④公网 E2E 实测 16 PASS/0 FAIL（scripts/test_v24_live.js：表头手机号/邮箱列+搜索 placeholder、接口 20 条 18 含手机号 0 脱敏样例 19943724100、表格渲染完整号码、无脱敏星号、手机号搜索命中 13612674128、无痕默认关灰底 rgb(209,213,219)、开启标记=0 重进紫底、关闭标记=1 重进灰底、无痕首页无首次提示）；APP 经 Capacitor server.url 直载线上页面，v24 网页部署即覆盖 APP 端无需重建 APK |
| Git HEAD | v25.0.47_24 代码提交 1aceff0（main，功能提交+总账收口提交；本地=GitHub=服务器源码仓=生产运行目录 四端一致；后台用户完整注册信息+屏幕放大默认关闭+发布脚本 release_v25_0_47_24.sh+E2E 测试脚本 test_v24_live.js） |
...
| 前端发布 | /root/yandaoguoxue/releases/&lt;tag&gt; + current 软链（SPA 静态导出）；当前 current→v25.0.47_24，回滚目标 v25.0.47_23；目录现状 _5/_6/_8/_9/_17/_21/_22/_23/_24 等多版（磁盘充足暂留，如需清理保留 _23+_24 即可） |
...
| ADMIN_CONTROL_CENTER（统一后台） | VERIFIED（v25.0.47_24 用户管理完整注册信息+16 E2E PASS） | /admin 全端统一抽屉式导航（v17 删除桌面常驻侧栏方案：默认收起+汉堡「打开导航菜单」唤出+遮罩覆盖，内容区全宽零避让，桌面/移动一致）；adminRoles.js 统一权限模块：SUPER_ADMIN/FINANCE_ADMIN/OPERATOR_ADMIN 服务端强校验（越权 403+审计 AUDIT_BLOCK_*），子密钥 SHA256 哈希存储；公网实测财务密钥越权 5 项全 403、运营密钥越权财务 403；密钥管理页 /admin/keys 签发/禁用子密钥；v25.0.47_24 用户管理（/admin/moderation 用户 tab）表格新增「手机号」「邮箱」列完整直出（后端去脱敏，接口鉴权 SUPPORT_ADMIN/ops，无密钥 401），搜索支持昵称/用户ID/手机号/邮箱四字段，E2E 16 PASS（完整号码 18 条 0 脱敏+手机号搜索命中） |
