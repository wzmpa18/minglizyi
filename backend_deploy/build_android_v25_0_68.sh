#!/bin/bash
# ============================================================================
# v25.0.68 APK 构建：易学四工具上线（versionCode 2068）
#   背景：Web 已发 v25.0.68（专业罗盘/七政四余/立极尺/鲁班尺丁兰尺四工具），
#         APK 需对齐重建，手机端才能收到 v25.0.68 升级引导。
#   前置：/root/yandaoguoxue/releases/v25.0.68 在产（current 软链指向）
#   流程：out/ ← releases/v25.0.68（与线上逐字节一致）→ build.gradle 2068/25.0.68
#         → assets/public + app-native.json(2068) → gradle assembleRelease
#         → APK 内容门禁（四工具页/SEO18页回归/版本烧录/签名/单一下载源）
#         → 原子替换 latest.apk → app-release-config.json 升 2068
#         → v25.0.68 升级公告 → 公网全链路验证
...
RELEASE_DIR="/root/yandaoguoxue/releases/v25.0.68"
...
grep -q "v25.0.68" "$RELEASE_DIR/version.json" || { echo "FATAL: release 非 v25.0.68"; exit 1; }
[ -x "$GRADLE_BIN" ] && [ -x "$AAPT" ] || { echo "FATAL: 构建工具链缺失"; exit 1; }
df -h / | tail -1

echo "--- [1] 同步线上 v25.0.68 资源到源码 out/（与生产 current 完全一致） ---"
rm -rf "$SRC_DIR/out"
mkdir -p "$SRC_DIR/out"
...
grep -q "v25.0.68" "$SRC_DIR/out/version.json" || { echo "FATAL: out/ 版本非 v25.0.68"; exit 1; }
...
echo "--- [5] APK 内容门禁（v25.0.68 四工具批次） ---"
...
grep -q "v25.0.68" assets/public/version.json || { echo "FATAL: 内置 web 资源版本错误"; exit 1; }
...
cat > /www/yandaoguoxue-backend/data/app-release-config.json <<EOCFG
{
  ...
  "releaseNotes": [
    "新增专业罗盘：WMM2025 磁偏角引擎，磁北/真北双模式，二十四山坐向实时判读",
    "新增七政四余：天文历算排盘，二十八宿宿度、洞微大限行限查询",
    "新增立极尺：户型图导入叠加罗盘，立极定向，一键导出",
    "新增鲁班尺/丁兰尺：多制式吉凶判读，双尺合参，吉尺寸建议",
    "内置资源与官网同步至 v25.0.68，功能完全一致"
  ],
  "forceUpdate": false,
...
echo "--- [8] 发布 v25.0.68 升级公告（替换过时版本公告） ---"
node <<'EONODE'
const fs = require('fs');
...
keep.unshift({
  id: 'a_v25_0_68_release',
  title: '🎉 新版 v25.0.68 发布：专业罗盘、七政四余等四大易学工具上线',
  content: '【本次更新内容】\n1. 新增专业罗盘：NOAA WMM2025 磁偏角引擎，支持磁北/真北切换、二十四山坐向实时判读、磁场干扰提示，兼向骑缝精准到 3°；\n2. 新增七政四余：专业天文历算排盘，今制/恒星制双星制，二十八宿宿度、命宫命度、洞微大限行限查询；\n3. 新增立极尺：户型图导入叠加罗盘盘面，对角线自动立极、点测定向，结果可导出分享（图纸仅本机处理不上传）；\n4. 新增鲁班尺/丁兰尺：现代通用/明清正宗多制式，分位吉凶判读、双尺合参、最近吉尺寸建议。\n\n老版本用户打开 APP 会收到升级引导，按提示操作即可完成升级。',
  level: 'important',
  pinned: true,
  published: true,
  publishAt: now,
...
echo "$A" | grep -q "v25.0.68" || { echo "FATAL: 公告未更新"; exit 1; }
echo "公告 OK → v25.0.68 升级公告已生效"
...
echo ""
echo "===== APK v25.0.68 BUILD COMPLETE（${VN}/${VC} 统一分发 latest.apk + 升级提示 + 公告 全部就绪） ====="
