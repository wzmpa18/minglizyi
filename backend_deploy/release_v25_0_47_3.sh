#!/bin/bash
# ============================================================================
# v25.0.47_3 前端发布：梅花伏神FU_SHEN_ROW渲染（本卦/互卦/变卦三卦层全覆盖）
# 产物 → releases/v25.0.47_3 → current 软链 → 同步源码仓 out/ → nginx 缓存清理 → 公网验证
# ============================================================================
set -euo pipefail
REL_TAG="v25.0.47_3"
RELEASE_DIR="/root/yandaoguoxue/releases/${REL_TAG}"
TAR="/root/yandaoguoxue/out_v25_0_47_3.tar.gz"
...
test -f "$RELEASE_DIR/yixue/meihua/index.html" || { echo "FATAL: meihua page missing"; exit 1; }
...
echo "[4] v25.0.47_3 内容门禁（构建产物功能标记）"
...
grep -rq "fushenLayer" "$RELEASE_DIR/_next/static/chunks/" && echo "FUSHEN-RENDER(梅花伏神渲染) OK" || { echo "FATAL: 梅花伏神渲染未入包"; exit 1; }
grep -rq "hiddenBranch" "$RELEASE_DIR/_next/static/chunks/" && echo "FUSHEN-DATA(伏神数据层) OK" || { echo "FATAL: 伏神数据层未入包"; exit 1; }
...
echo "[6] 公网验证"
for p in "" discover yixue/meihua yixue/liuyao groups admin/sources; do
