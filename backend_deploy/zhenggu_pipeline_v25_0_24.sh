#!/bin/bash
# 中华非遗正骨全链路流水线 v25.0.24：解析(#40-54) → 知识点审核 → 全覆盖出题 → 覆盖率验收
# 用法: nohup bash /root/zhenggu_pipeline_v25_0_24.sh > /root/zhenggu_pipeline_v25_0_24.log 2>&1 &
set -u
...
log "==== 正骨流水线启动（15部 / 类目=$CAT）===="

# 阶段1：逐部解析 #40-54（串行，每部等待解析完成）
for MID in $(seq 40 54); do
