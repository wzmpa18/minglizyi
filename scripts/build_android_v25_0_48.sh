#!/bin/bash
# ============================================================================
# v25.0.48 APK 重建：FIX-V16 升级提示 + 海报/导航修复内置资源更新
#   前置：已执行 deploy_release_v25_0_47_16.sh（out/ 为最新构建产物）
#   流程：out/ → android assets/public/ → 写入 app-native.json（versionCode 2048）
...
NEW_APK_NAME="yandao-guoxue-v25.0.48-release.apk"

cd "$SRC_DIR"

echo "--- [0] 前置校验 ---"
test -f out/index.html || { echo "FATAL: out/ 不存在，请先执行 deploy_release_v25_0_47_16.sh"; exit 1; }
BUILD_ID=$(node -e "console.log(JSON.parse(require('fs').readFileSync('out/version.json','utf8')).buildId)")
echo "web资源 buildId: ${BUILD_ID}"
echo "$BUILD_ID" | grep -q "v25.0.47_16" || { echo "FATAL: out/ 非 v25.0.47_16 构建"; exit 1; }
grep -q "发现新版本" out/_next/static/chunks/*.js 2>/dev/null && echo "升级弹窗已入 web 资源 OK" || { echo "FATAL: web 资源缺少升级弹窗代码"; exit 1; }
...
cat > "$ASSETS_PUBLIC/app-native.json" <<'EON'
{
  ...
  "versionCode": 2048,
  "platform": "android",
  "builtAt": "2026-08-23T16:00:00+08:00"
}
EON
...
grep -q '"versionCode": 2048' assets/public/app-native.json || { echo "FATAL: 内置版本号错误"; exit 1; }
...
EOCFG
grep -q '"latestVersionCode": 2048' /www/yandaoguoxue-backend/data/app-release-config.json || { echo "FATAL: 后端版本配置写入失败"; exit 1; }
...
echo "===== APK v25.0.48 BUILD COMPLETE ====="
echo "NOTE: 旧 v25.0.47 APK 保留在分发目录作回滚备份（不再被前端引用）"
