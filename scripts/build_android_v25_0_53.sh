#!/bin/bash
# ============================================================================
# v25.0.53 APK 构建：DEV-V22-PARTNER-V2-BACKSWIPE 合伙人渠道体系V2 + 双侧侧滑返回
#   前置：已执行 bash build.sh（out/ 为 v25.0.47_26 构建产物）
#   流程：out/ → android assets/public/ → 写入 app-native.json（versionCode 2053）
#         → gradle assembleRelease（自动签名）→ 验证 → 上传分发目录 → 更新别名
#         → latest.apk 永久固定名别名（全站 APK 直链统一指向这一处）
#         → 后端 app-release-config.json 更新至 2053（升级提示数据源）
# ============================================================================
set -e
...
NEW_APK_NAME="yandao-guoxue-v25.0.53-release.apk"
...
echo "$BUILD_ID" | grep -q "v25.0.47_26" || { echo "FATAL: out/ 非 v25.0.47_26 构建"; exit 1; }
grep -q 'versionCode 2053' android/app/build.gradle || { echo "FATAL: build.gradle versionCode 非 2053"; exit 1; }
grep -q 'versionName "25.0.53"' android/app/build.gradle || { echo "FATAL: build.gradle versionName 非 25.0.53"; exit 1; }
...
grep -q '"versionCode": 2053' assets/public/app-native.json || { echo "FATAL: 内置版本号错误"; exit 1; }
echo "--- 内置 version.json ---"
cat assets/public/version.json
grep -q "v25.0.47_26" assets/public/version.json || { echo "FATAL: 内置 web 资源版本错误"; exit 1; }
echo "--- 内置关键代码检查 ---"
unzip -o -q "$APK_OUT" "assets/public/_next/static/chunks/*" 2>/dev/null
for kw in "申请成为渠道合伙人" "累计培养奖励" "我的合伙人" "发现新版本" "renderViralPoster" "latest.apk"; do
...
{
  "latestVersion": "25.0.53",
  "latestVersionCode": 2053,
  "downloadUrl": "https://yandaoguoxue.yandao.vip/app-download/latest.apk",
  "downloadPage": "https://yandaoguoxue.yandao.vip/friend",
  "releaseNotes": [
    "新增合伙人渠道体系：专属邀请码与推广海报，渠道用户、佣金与培养奖励明细随时可查",
    "支持推荐合伙人建立团队：直属一级培养奖励，扫码自动绑定推荐关系",
    "全页面新增侧滑返回手势：屏幕左右边缘轻扫即可返回上一页，操作更顺手",
    "性能与稳定性优化：会员、排盘、AI 解读等原有功能全面回归验证"
  ],
  "forceUpdate": false,
  "publishedAt": "${BUILT_AT}"
}
EOCFG
grep -q '"latestVersionCode": 2053' /www/yandaoguoxue-backend/data/app-release-config.json || { echo "FATAL: 后端版本配置写入失败"; exit 1; }
...
pm2 restart yandaoguoxue-backend --update-env > /dev/null 2>&1
sleep 3
curl -s -m 10 https://yandaoguoxue.yandao.vip/api/public/app-version | grep -q '"latestVersionCode":2053\|"latestVersionCode": 2053' || { echo "FATAL: 升级接口未返回2053"; exit 1; }
echo "后端配置 OK（升级接口已返回 2053）"
...
SAME=$(md5sum "$DIST_DIR/latest.apk" "$DIST_DIR/$NEW_APK_NAME" | awk '{print $1}' | uniq -d | wc -l)
[ "$SAME" -eq 1 ] && echo "latest.apk 与新包 MD5 一致 OK" || { echo "FATAL: latest.apk 与新包 MD5 不一致"; exit 1; }

echo "===== APK v25.0.53 BUILD COMPLETE ====="
