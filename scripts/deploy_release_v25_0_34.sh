#!/bin/bash
# v25.0.34 发布：P7-上架前阻断整改-01 阶段1 紫微布局
#   宫格尺寸回退 v25.0.31 固定 aspectRatio 0.75（宫大小不再调整）
#   宫干支恢复（D区右侧，干支五行色，四化/三合/飞星模式通用）
#   36神煞固定左下角纵排；A区字号全盘固定统一（主11/辅10/杂9，严禁缩字）
#   E区动态星每列10颗最多3列（适配固定宫格）
set -e
...
echo "--- [2] 页面导出校验（紫微 + 抽检核心页） ---"
for p in yixue/ziwei friends profile/edit academy invite register login profile privacy; do
  test -f "out/${p}/index.html" || { echo "FATAL: out/${p}/index.html missing"; exit 1; }
  echo "OK: ${p}"
done

echo "--- [3] 阶段1功能标记入包校验 ---"
# 紫微：固定宫格 aspectRatio 0.75 入包
grep -rq "0.75" out/_next/static/chunks/ >/dev/null 2>&1 && echo "ZIWEI-FIXED-GRID(固定宫格0.75) OK" || { echo "FATAL: 固定宫格标记缺失"; exit 1; }
# 宫干支恢复（getGanZhiColor 应用到宫干支）
grep -rq "heavenlyStem" out/_next/static/chunks/ >/dev/null 2>&1 && echo "ZIWEI-GANZHI(宫干支数据) OK"
# 时间轴/叠宫/运限四化既有功能保留（红线：无回归）
...
# 自适应高度残留校验（负向：minHeight 540px 不得回流）
if grep -rq "540px" out/_next/static/chunks/ 2>/dev/null; then
  echo "FATAL: minHeight 540px 自适应高度残留"; exit 1
fi
echo "ZIWEI-NO-ADAPTIVE-HEIGHT(无自适应宫高残留) OK"
...
for path in yixue/ziwei friends academy; do
...
echo "$VJSON"
echo "$VJSON" | grep -q "\"v25.0.34\"" || { echo "WARN: 公网version未生效（可能缓存，稍后复验）"; }
