#!/bin/bash
# ============================================================================
# v25.0.47_17 发布：FIX-V17 全端抽屉导航+会员入口加固+动态版本号+典籍AI全量导入
#   ① 后台导航全端统一抽屉式（删除桌面端常驻侧栏：默认收起+汉堡+遮罩覆盖，内容区全宽）
#   ② 商业中心默认展开+会员中心原生锚点跳转（不依赖JS，杜绝"点了没反应"）
#   ③ 版本号动态化（IcpFooter/关于弹窗读 version.json）+系统中心"检查更新"手动升级按钮
#   ④ 中医典籍混元AI全量导入：7部典籍章节补全+新增6部典籍（classicsExtra 外挂模块）
# 执行顺序 = ① bash build.sh ② bash scripts/build_android_v25_0_49.sh ③ 本脚本
# ============================================================================
set -e
...
echo "--- [1] 内容门禁（v13~v16 全量保留 + v17 新增） ---"
# ===== 存量门禁（v13 全量） =====
...
grep -q 'versionCode 2049' android/app/build.gradle || { echo "FATAL: APK versionCode未升级到2049"; exit 1; }
# ===== v17 新增门禁（全端抽屉+会员入口+动态版本+典籍导入） =====
...
echo "synced: 后端16文件（app-release-config 由 APK 构建脚本负责更新至 2049）"
...
for path in membership zhongyi zhongyi/classic yixue yixue/phone academy/yikao profile index admin admin/feature-flags admin/tool-control admin/pricing admin/commission admin/keys register login invite invite/poster friend download api/health; do
  CODE=$(curl -sL -o /dev/null -w '%{http_code}' ${DOMAIN}/${path})
  echo "公网 /${path}: ${CODE}"
  [ "$CODE" != "200" ] && { echo "FATAL: /${path} 公网非200"; exit 1; }
done

echo "--- [8.5] 公网版本号+典籍内容验证（v17核心） ---"
echo "version.json: $(curl -s -m 10 ${DOMAIN}/version.json | tr -d '\n')"
VJ=$(curl -s -m 10 ${DOMAIN}/version.json)
echo "$VJ" | grep -q 'v25.0.47_17' || { echo "FATAL: 公网版本号未更新到v25.0.47_17"; exit 1; }
...
echo "NOTE: 执行顺序 = ① bash build.sh ② bash scripts/build_android_v25_0_49.sh（更新app-release-config至2049） ③ 本脚本（网页版切流）"
