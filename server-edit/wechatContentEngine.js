...
const DEFAULT_SETTINGS = {
  ...
  maxBatchesPerMonth: 4,       // 每月最多4个内容生产批次（FINAL-17 第十六章）
  maxDraftsPerBatch: 5,        // 每批最多5篇候选草稿（FINAL-17 第十六章）
  batchDays: '1,8,15,22',      // 每月批次执行日（FINAL-17 第十六章）
  pendingReviewPause: 5,       // 待审草稿≥5 暂停AI正文生成（FINAL-17 第十八章）
  maxArticleTokens: 6000,
  dailyCostCap: 20,            // CNY/日 成本保护（第八十三章）
  topicTopN: 8,                // 每批选题 TOP N（第四十一章）
  topicQualityFloor: 40,       // 批次选题质量底线：final_score≥40 才算好选题（FINAL-17 第十七章宁缺毋滥）
  authorName: '言道国学',
...
// ---------- 批次门禁（FINAL-17 第十六/十八/二十章：每月4批、每批≤5篇、待审≥5暂停） ----------
function beijingToday() { return new Date(Date.now() + 8 * 3600 * 1000).toISOString().slice(0, 10); }
function isBatchDay(d) {
  const day = Number(String(d || beijingToday()).slice(8, 10));
  const days = String(settings().batchDays || '1,8,15,22').split(',').map((x) => Number(x.trim()));
  return days.includes(day);
}
// 本月已消耗批次 = 本月有文章产出的去重天数
function monthlyBatchCount(d) {
  const month = String(d || beijingToday()).slice(0, 7);
  const row = getDb().prepare("SELECT COUNT(DISTINCT date(created_at)) AS n FROM wechat_articles WHERE substr(created_at,1,7) = ?").get(month);
  return row.n;
}
// 待审草稿数（已同步微信草稿箱、等待Owner人工审核）
function pendingReviewCount() {
  return getDb().prepare("SELECT COUNT(*) AS n FROM wechat_articles WHERE status = 'WECHAT_DRAFT'").get().n;
}
...
// 批次质量门禁自动批准（FINAL-17 第十七章：final_score≥质量底线才算好选题；不足5个就少生成，宁缺毋滥）
function autoApproveBatchTopics(runDate) {
  const s = settings();
  const floor = Number(s.topicQualityFloor) || 40;
  const candidates = listTopics(runDate).filter((t) => t.status === 'PENDING');
  let approved = 0;
  const db = getDb();
  for (const t of candidates) {
    if (approved >= Number(s.maxDraftsPerBatch)) break;
    if (Number(t.final_score) < floor) continue;
    db.prepare("UPDATE wechat_topic_candidates SET status = 'APPROVED', updated_at = datetime('now','localtime') WHERE topic_id = ?").run(t.topic_id);
    approved++;
  }
  return approved;
}
...
function dashboardStats() {
  ...
  const pendingReview = pendingReviewCount() + db.prepare("SELECT COUNT(*) AS n FROM wechat_articles WHERE status = 'SAFETY_PASSED' AND wechat_media_id = ''").get().n;
...
module.exports = {
  ...
  isBatchDay, monthlyBatchCount, pendingReviewCount, autoApproveBatchTopics,
