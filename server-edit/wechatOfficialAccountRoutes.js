...
function createRouter() {
  ...
  router.post('/admin/generate', adminAuth('OPERATOR_ADMIN'), async (req, res) => {
    try {
      ...
      const limit = contentEngine.settings().maxDraftsPerBatch;
      const results = [];
  ...
  router.put('/admin/settings', adminAuth('ADMIN'), (req, res) => {
    const allowed = ['automation', 'draftSync', 'maxBatchesPerMonth', 'maxDraftsPerBatch', 'batchDays', 'pendingReviewPause', 'topicQualityFloor', 'maxArticleTokens', 'dailyCostCap', 'topicTopN', 'authorName', 'ctaText', 'keywordBlacklist', 'coverTemplate'];
    const patch = {};
    for (const k of allowed) if (req.body[k] !== undefined) patch[k] = req.body[k];
    if (patch.maxDraftsPerBatch) patch.maxDraftsPerBatch = Math.max(1, Math.min(5, Number(patch.maxDraftsPerBatch) || 5));
    if (patch.maxBatchesPerMonth) patch.maxBatchesPerMonth = Math.max(1, Math.min(4, Number(patch.maxBatchesPerMonth) || 4));
  ...
  router.get('/admin/notifications', adminAuth('OPERATOR_ADMIN'), (req, res) => {
    const stats = contentEngine.dashboardStats();
    res.json({ success: true, data: { pendingReview: stats.pendingReview, riskBlocked: stats.riskBlocked, message: stats.pendingReview > 0 ? `当前有 ${stats.pendingReview} 篇公众号草稿待审核（≥5 将暂停AI正文生成）` : '' } });
  });
