/**
 * 原始来源：净室自研，MIT License
 * 原始版本：v1.0
 * 修改记录：2026-07-26 净室重写，基于公开命理经典独立构建，51种神煞
 * 当前协议：MIT
 * 参考依据：《渊海子平》《三命通会》神煞章节
 * 净室声明：所有神煞计算规则基于公开经典文献独立推导，未参考任何 AGPL 源码
 */
 ...
 * 获取六十甲子索引
 */
export function jiaziIndex(ganzhi) {
 ...
 */
export function getSeason(monthZhi) {
  var seasonMap = {
    '寅': '春', '卯': '春', '辰': '春',
    '巳': '夏', '午': '夏', '未': '夏',
    '申': '秋', '酉': '秋', '戌': '秋',
...
export const SHENSHA_DEFINITIONS = {
 ...
 */
export function _checkGanShenSha(name, def, refGan, allGanZhi) {
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkZhiShenSha(name, def, refZhi, allGanZhi) {
  if (!def || !def.data) return [];
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkZhiSanHeShenSha(name, def, refZhi, allGanZhi) {
  if (!def || !def.data) return [];
  var targetZhi = def.data[refZhi];
  if (!targetZhi) return [];
  var found = [];
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkGanZhiShenSha(name, def, allGanZhi) {
  if (!def || !def.data) return [];
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkSeasonShenSha(name, def, season, allGanZhi) {
  if (!def || !def.data) return [];
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkSeasonMultiShenSha(name, def, season, allGanZhi) {
  if (!def || !def.data) return [];
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkMonthZhiShenSha(name, def, monthZhi, allGanZhi) {
  if (!def || !def.data) return [];
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkMonthZhiShiftShenSha(name, def, monthZhi, allGanZhi) {
  if (!def || !def.data) return [];
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkSanQi(allGan) {
  var patterns = [
    ['甲', '戊', '庚'],
    ['乙', '丙', '丁'],
    ['壬', '癸', '辛']
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkYuanChen(yearGan, yearZhi, gender, allGanZhi) {
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkKongWang(dayGanZhi, allGanZhi) {
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkFixedShenSha(def, gender, allGanZhi) {
  if (!def || !def.data) return [];
  var data = def.data;
  var targetZhi;
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkFuYin(allGanZhi) {
  var found = [];
  var seen = {};
  for (var i = 0; i < allGanZhi.length; i++) {
    var gz = allGanZhi[i];
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkFanYin(allGanZhi) {
  var found = [];
  for (var i = 0; i < allGanZhi.length; i++) {
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkSiZheng(allGanZhi) {
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkSiSheng(allGanZhi) {
 ...
 * @returns {string[]} 所在柱位列表
 */
export function _checkCiGuan(dayGan, allGanZhi) {
 ...
 * }
 */
export function calculateAllShenSha(bazi) {
 ...
 */
export function getShenShaByPillar(bazi, pillarName) {
 ...
 */
export function getShenShaByCategory(bazi, category) {
  var result = calculateAllShenSha(bazi);
 ...
 * @returns {boolean}
 */
export function hasShenSha(bazi, name) {
  var result = calculateAllShenSha(bazi);
 ...
 * 51种神煞分类清单（用于展示和校验）
 */
export const SHENSHA_CATEGORY_LIST = {
  '吉': [
