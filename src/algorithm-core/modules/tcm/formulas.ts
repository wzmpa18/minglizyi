...
/**
 * 按中药名搜索包含该药的方剂
 * 在方剂组成（composition）中匹配中药名，支持精确匹配和包含匹配
 * @param herbName 中药名称
 * @returns 包含该中药的方剂列表
 */
export function searchFormulasByHerb(herbName: string): TcmFormula[] {
  if (!herbName || herbName.trim() === '') return [];
  const db = fullFormulasLoaded ? fullFormulasDB : FORMULAS_DB;
  const name = herbName.trim();
  return db.filter((f) =>
    f.composition.some(
      (c) => c.herb === name || c.herb.includes(name) || name.includes(c.herb)
    )
  );
}
