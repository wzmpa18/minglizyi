...
export default function HehunPage() {
  ...
  useEffect(() => {
    ...
    // P1-08 修复：hehunResult 为 useMemo，在 hasResult 变化后才会重算
  ...
  const doHehun = useCallback(() => {
    ...
    setTimeout(() => {
      setHasResult(true);
      setLoading(false);
      // P1-08 修复：savePaipanState 已移入 useEffect，此处不再调用
      // hehunResult 为 useMemo，需等 hasResult 变更后重新计算才有效
      // 滚动到结果区
      setTimeout(() => {
        ...
        <div className="mt-2">
          <PostToSquareButton tool="八字合婚" summary="合婚分析已完成，双方八字匹配度已出" />
            ...
            <EventDivinationPanel
              toolName="八字合婚"
              chartContext={`男方八字: ${hehunResult.male.pillars.map((p:any)=>p.gan+p.zhi).join(" ")}\n女方八字: ${hehunResult.female.pillars.map((p:any)=>p.gan+p.zhi).join(" ")}\n合婚评分: ${hehunResult.totalScore}分 ${hehunResult.grade}\n${hehunResult.items.map(i => i.name+":"+i.passDesc).join("; ")}\n总评: ${hehunResult.summary}`}
              isPaidTool={false}
            />
          </div>
        )}
