...
export default function AIAssistant() {
  ...
  const welcomeText = isZhongyi
    ? `${DISCLAIMER}\n\n你好！我是中医助手，可以帮你解答中医学习中的方剂、药材、经络等问题。请随时向我提问。`
    : `${DISCLAIMER}\n\n你好！我是易学助手，可以帮你解答八字、紫微、奇门、六爻等命理学习问题。请随时向我提问。`;
  ...
  const handleSend = async () => {
    ...
    if (!perm.canUseAI) {
      const msg: ChatMessage = {
        id: genId(),
        role: "ai",
        content: perm.message || "今日AI解读次数已用完，开通会员继续使用",
        time: now(),
      };
    ...
    try {
      const systemPrompt = isZhongyi
        ? "你是专业的中医学习助手，精通方剂、中药、经络、针灸、中医基础理论等。请用中文回答用户关于中医学习的问题，内容专业、准确、有条理。回答开头需附上免责声明「AI 生成内容仅供学习参考，不构成任何建议」。"
        : "你是专业的易学学习助手，精通八字、紫微斗数、奇门遁甲、六爻、梅花易数等传统命理学。请用中文回答用户关于易学学习的问题，内容专业、准确、有条理。回答开头需附上免责声明「AI 生成内容仅供学习参考，不构成任何建议」。";

      const result = await callAI({
        systemPrompt,
        userPrompt: text || "请解读这张截图的内容",
        cacheKey: `${isZhongyi ? "zhongyi" : "yixue"}_assistant_${text.slice(0, 60)}`,
      });

      const aiContent = result.success
        ? result.content
        : "暂时无法连接AI服务，请稍后再试";
